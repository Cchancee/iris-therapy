...
import TherapistIcon from '@mui/icons-material/Person';
...
export default function MiniDrawer() {
  ...
  const pages = [
    { name: 'Dashboard', path: '/patient', icon: <DashboardIcon /> },
    { name: 'Appointments', path: 'appointments', icon: <AppointmentsIcon /> },
    { name: 'Therapists', path: 'therapists', icon: <TherapistIcon /> },
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      ...
      <AppBar position="fixed" open={open}>
        <Toolbar>
          ...
          <Box sx={{ display: { xs: 'none', md: 'flex', backgroundColor: '#fff', borderRadius: 30, paddingInline: 10, paddingBlock: 2, marginRight: 10 } }}>
            ...
            >
              <img src={profile} alt="" className='max-w-[20px] max-h-[20px] rounded-full' />
            </IconButton>
          </Box>
      ...
      <Drawer variant="permanent" open={open} sx={{ backgroundColor: '#dce2f3', zIndex: 100, position: { xs : open ? 'fixed' : "relative", sm: 'relative'} , isolation: "isolate" }}>
        <DrawerHeader sx={{ backgroundColor: '#fff', boxShadow: 'none', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div></div>
            ...
            <ListItem key={name} disablePadding sx={{ display: 'block', color: '#333' }}>
              <ListItemButton
                sx={{
                  ...
                  backgroundColor: isActive(path) ? '#dce2f3' : 'transparent', // Active link background
                  borderRight: isActive(path) ? '5px solid #8FA5FF' : 'none', // Active link border
                }}
                component={Link}
                to={path.startsWith('/') ? path : `${basePath}/${path}`} // Construct full path for relative ones
              >
                <ListItemIcon
                  sx={[
                  {
